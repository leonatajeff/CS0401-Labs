...
public class Lab7
{
	...
	public static void loadData(MovieDB movies) throws IOException
	{
		...
		File inFile = new File("movieFile.txt");
		Scanner fileIn = new Scanner(inFile);
		int num_Movies = fileIn.nextInt(); fileIn.nextLine();
		for (int k = 0; k < num_Movies; k++) {
			
		String movie_Title = fileIn.nextLine();
		String director = fileIn.nextLine();
		String studio = fileIn.nextLine(); 
		String gross = fileIn.nextLine();
		double numGross = Double.parseDouble(gross);
	
		
		Movie movie = new Movie(movie_Title, director, studio, numGross);
	...
	public static void getCommands(MovieDB movies)
	{
		...
		while (true)
		{
			...
			else if (choice.equals("2"))
			{
				...
				String movie_Title = inScan.nextLine();
				System.out.println("Director?\n");
				String director = inScan.nextLine();
				System.out.println("Studio?\n");
				String studio = inScan.nextLine();
				System.out.println("Gross?\n");
				double gross = own.nextDouble();
				
				Movie add_movie = new Movie(movie_Title, director, studio, gross);
				movies.addMovie(add_movie);
				// *** CODE SEGMENT 2 *** //
			...
			{
				System.out.println("Movie name?\n");
				String movie_Title_2 = inScan.nextLine();
				if (movies.findMovie(movie_Title_2) == null) {
					System.out.println("\nMovie not found!\n");
				}
				else {
					System.out.println("\nMovie found!\n");
					movies.findMovie(movie_Title_2).toString();
				}
